...
COLUMN_MAP_FILE = "Execution_IntAss_ColumnMap.json"
DB_FILE = "db.json"
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) >= 3:
            ...
            print(f"Running IA IPE Testing with custom keys: {ipe_custom_keys}")
            print(f"Reference value: {reference_value}")
            
            # Run the jugg function
            jugg(TARGET, reference_value, ipe_custom_keys)
            
            result = {"success": True, "message": "IA IPE Testing completed successfully"}
