...
def run_module(module_id, config):
    ...
    try:
        ...
        if module_id == "get_column":
            # Column mapping generation (prerequisite for other modules)
            ...
            column_map_path = os.path.join(output_dir, "column_map.json")
            logging.info(f"IPE Testing - Column map path: {column_map_path}")
            ...
            if not os.path.exists(column_map_path):
                ...
                logging.info("Column map generation completed")
            else:
                logging.info("Using existing column map")
            ...
            logging.info(f"IPE custom keys: {ipe_custom_keys}")
...
if __name__ == "__main__":
    try:
        ...
        logging.info(f"Module ID: {module_id}")
