...
export default function ActuaryTesting({ onBack }: ActuaryTestingProps) {
  ...
  const loadColumnsForFile = async (fileName: string, target: "ctc" | "actuary") => {
    ...
    if (res.ok && Array.isArray(res.columns)) {
      if (target === "ctc") setCtcColumns(res.columns as string[]);
      if (target === "actuary") setActuaryColumns(res.columns as string[]);
    }
  };
  ...
  const runActuaryTesting = async () => {
    ...
    setErrorMessage("");
    try {
      // Build mapping exactly as required: [{"CTC":"...","Actuary":"..."}]
      const column_map = mappings
        .filter(m => m.ctc && m.actuary)
        .map(m => ({ CTC: String(m.ctc), Actuary: String(m.actuary) }));
      const input_mapping = { column_map };
      // sanitize filenames to remove appended reference in UI selection
      ...
      if (window.payroll?.run) {
        // Pass a flat object where input_mapping is fully materialized, not [Object]
        const res = await window.payroll.run("execute_actuary_testing", {
          inputFiles: [],
          options: {
            ctc_file: ctcFileOnly,
            actuary_file: actuaryFileOnly,
            // Ensure column_map is serializable and visible in logs
            input_mapping: { column_map: JSON.parse(JSON.stringify(column_map)) },
          },
        });

        if (res.ok && res.runId) {
          // Hard timeout to avoid indefinite spinner (5 minutes)
          const timeoutId = setTimeout(() => {
            setProcessingStatus("error");
            setErrorMessage("Operation timed out. Please verify SharePoint connectivity and file names, then try again.");
          }, 5 * 60 * 1000);
          const off = window.payroll.onProgress((payload: any) => {
            if (payload.runId !== res.runId) return;
            if (payload.status === "running") {
              setProgress(prev => Math.min(95, Math.max(5, prev + 5)));
              // Try parse early JSON
              try {
                if (payload.stdout) {
            ...
            if (payload.status === "success") {
              try {
                if (payload.stdout) {
                  ...
                  if (jsonLine) {
                    const parsed = JSON.parse(jsonLine);
                    if (parsed.success && (parsed.excel_url || parsed.json_url)) {
                      setProcessingStatus("completed");
                      setProgress(100);
                      if (parsed.excel_url) setExcelUrl(parsed.excel_url);
                      if (parsed.json_url) setJsonUrl(parsed.json_url);
                      ...
                      setProcessingStatus("error");
                      setErrorMessage(parsed.error || "No outputs generated. Check mappings and file names.");
                    }
                  } else {
                    // No JSON in stdout; treat as error to avoid silent success
                    setProcessingStatus("error");
                    setErrorMessage("No completion signal received from runner.");
                  }
                } else {
                  setProcessingStatus("error");
                  setErrorMessage("Runner produced no output.");
                }
              } catch {
                setProcessingStatus("error");
                setErrorMessage("Failed to parse runner output.");
              }
              clearTimeout(timeoutId);
              off();
              ...
              if (payload.error) details.push(String(payload.error));
              if (payload.stderr) details.push(String(payload.stderr));
              if (payload.stdout) details.push(String(payload.stdout));
              const combined = details.filter(Boolean).join("\n\n").slice(0, 4000);
              if (combined) setErrorMessage(combined);
              clearTimeout(timeoutId);
              off();
            }
          });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Step 1: Select input files (reuse Exception Testing pattern) */}
      <Card className="border-white/10 bg-black/40">
        <CardHeader>
          <CardTitle className="flex items-center gap-3 text-white">
            <Settings className="h-5 w-5" />
            Step 1: Select CTC Report & Actuary File
          </CardTitle>
        </CardHeader>
        ...
        <CardContent className="space-y-3">
                  ...
                  <TableRow key={row.id}>
                    <TableCell>
                      <Select value={row.ctc || ""} onValueChange={(v) => updateRow(row.id, "ctc", v)}>
                        <SelectTrigger className="border-white/10 bg-black/40 text-white">
                          <SelectValue placeholder="Select CTC column..." />
                        </SelectTrigger>
                        <SelectContent className="border-white/10 bg-black/90 text-white max-h-64">
                          {ctcColumns.map((c) => (
                    ...
                    <TableCell>
                      <Select value={row.actuary || ""} onValueChange={(v) => updateRow(row.id, "actuary", v)}>
                        <SelectTrigger className="border-white/10 bg-black/40 text-white">
                          <SelectValue placeholder="Select Actuary column..." />
                        </SelectTrigger>
                        <SelectContent className="border-white/10 bg-black/90 text-white max-h-64">
                          {actuaryColumns.map((c) => (
                            <SelectItem key={c} value={c}>{c}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
          ...
          {!isValid() && (
            <div className="flex items-center gap-2 text-xs text-red-300">
              ...
              Please select both files and map at least one pair of columns (both sides per row).
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="border-white/10 bg-black/40">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">Execute CTC vs Actuary reconciliation using your mapping</p>
            </div>
          ...
          {processingStatus === "error" && (
            <div className="rounded border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-200">
              {errorMessage || "An error occurred while processing. Please check inputs and try again."}
            </div>
          )}
            ...
            <div className="rounded border border-green-500/20 bg-green-500/10 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-200">Outputs uploaded to SharePoint</p>
                </div>
              </div>
              <div className="mt-3 flex gap-2">
                <Button variant="outline" className="border-green-500/30 text-green-300" onClick={() => excelUrl && window.open(excelUrl, "_blank")} disabled={!excelUrl}>
                  <Download className="mr-2 h-4 w-4" />
                  Open Excel Output
                </Button>
                <Button variant="outline" className="border-green-500/30 text-green-300" onClick={() => jsonUrl && window.open(jsonUrl, "_blank")} disabled={!jsonUrl}>
