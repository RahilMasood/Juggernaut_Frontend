...
interface IAExceptionTestingProps {
  onBack?: () => void;
}
...
export default function IAExceptionTesting({ onBack }: IAExceptionTestingProps) {
  ...
  const runExceptionTests = async () => {
    ...
    try {
      if (window.payroll?.run) {
        const result = await window.payroll.run("execute_ia_exception_testing", {
          inputFiles: [],
          options: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Exception Testing - Intangible Assets</h2>
