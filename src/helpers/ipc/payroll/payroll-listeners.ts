...
export function addPayrollEventListeners(mainWindow: BrowserWindow) {
  ...
  // Load Excel columns from SharePoint file via Python (pandas)
  ipcMain.handle(PAYROLL_LOAD_EXCEL_COLUMNS_CHANNEL, async (_evt, { fileName }: { fileName: string }) => {
    try {
      const pythonCmd = await findPythonExecutable();
      const cfgPath = path.join(getRunsDir(), `${Date.now()}_load_cols.json`);
      await fsp.writeFile(cfgPath, JSON.stringify({ file_name: fileName, folder_name: "client" }, null, 2), "utf-8");
      const scriptPath = path.join(process.cwd(), "scripts", "load_excel_columns_sharepoint.py");
      const child = spawn(pythonCmd, [scriptPath, cfgPath]);
      let stdout = "";
      let stderr = "";
      await new Promise<void>((resolve, reject) => {
